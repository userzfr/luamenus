...
HoaxMenu = NativeUI.CreateMenu("~h~TIAGO MENU", "¦ ~b~By Hoax", X, Y)

HoaxMenuPool = NativeUI.CreatePool()
HoaxMenuPool:Add(HoaxMenu)
...
function SpawnWeaponMenu(WeaponSpawnMenu, PlayerPed)
    ...
    local handGunsMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Handguns"))
    handGunsMenu.Item:RightLabel("→→")
    handGunsMenu = handGunsMenu.SubMenu

    local riflesMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Assault Rifles"))
    riflesMenu.Item:RightLabel("→→")
    riflesMenu = riflesMenu.SubMenu

    local shotgunsMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Shotguns"))
    shotgunsMenu.Item:RightLabel("→→")
    shotgunsMenu = shotgunsMenu.SubMenu

    local smgsMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Sub-/Light Machine Guns"))
    smgsMenu.Item:RightLabel("→→")
    smgsMenu = smgsMenu.SubMenu

    local throwablesMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Throwables"))
    throwablesMenu.Item:RightLabel("→→")
    throwablesMenu = throwablesMenu.SubMenu

    local meleeMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Melee"))
    meleeMenu.Item:RightLabel("→→")
    meleeMenu = meleeMenu.SubMenu

    local heavyMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Heavy Weapons"))
    heavyMenu.Item:RightLabel("→→")
    heavyMenu = heavyMenu.SubMenu

    local snipersMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "Sniper Rifles"))
    ...
    for key, weapon in pairs(weaponList) do
        ...
        if not string.IsNullOrEmpty(weapon.Name) then
            ...
            local weaponMenu = (HoaxMenuPool:AddSubMenu(WeaponSpawnMenu, "~y~" ..tostring(weapon.Name).."~s~ Options", weapon.Name, "false"))
...
do
    ...
    Citizen.CreateThread(function()
        while ShowMenu do
            ...
            HoaxMenu.Logo:Colour(color.r, color.g, color.b, 255)
        end
    end)
    ...
    local function AddPlayerOptionsMenu(menu)
        local PlayerOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "Player Options"))
        ...
        openSkinMenu.Activated = function(ParentMenu, SelectedItem)
            FiveM.TriggerCustomEvent(false, 'esx_skin:openRestrictedMenu', function(data, menu) HoaxMenuPool:CloseAllMenus() end) end
    ...
    local function AddOnlinePlayersMenu(menu)
        ...
        local OnlinePlayersMenu = (HoaxMenuPool:AddSubMenu(menu, "Online Players"))
        OnlinePlayersMenu.Item:RightLabel("→→")

        local OnlinePlayerOptionsMenu = HoaxMenuPool:AddSubMenu(OnlinePlayersMenu.SubMenu, "Online Player Options")
        ...
        local EasyAdmin = (HoaxMenuPool:AddSubMenu(OnlinePlayerOptionsMenu.SubMenu, "EasyAdmin → Triggers", "EasyAdmin Triggers"))
        ...
        local ESXTriggers = (HoaxMenuPool:AddSubMenu(OnlinePlayerOptionsMenu.SubMenu, "ESX → Triggers", "ESX Based Triggers"))
    ...
    local function AddTeleportMenu(menu)
        local TeleportMenu = (HoaxMenuPool:AddSubMenu(menu, "Teleport Options"))
    ...
    local function AddWeaponOptionsMenu(menu)
        local WeaponOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "Weapon Options"))
        WeaponOptionsMenu.Item:RightLabel("→→")
        WeaponOptionsMenu = WeaponOptionsMenu.SubMenu

        local WeaponSpawnMenu = (HoaxMenuPool:AddSubMenu(WeaponOptionsMenu, "Spawn Weapon"))
    ...
    local function AddVehicleOptionsMenu(menu)
        local VehicleOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "Vehicle Options"))
    ...
    local function AddTriggerOptionsMenu(menu)
        local TriggerOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "ESX Triggers"))
        ...
        ESXBoss.OnListSelected = function(menu, item, index)
            FiveM.TriggerCustomEvent(false, 'esx_society:openBossMenu', Jobs.Type[ESXBoss:IndexToItem(index)], function(data, menu) HoaxMenuPool:CloseAllMenus() end) end
    ...
    local function AddWorldOptionsMenu(menu)
        local WorldOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "World Options"))
        WorldOptionsMenu.Item:RightLabel("→→")
        WorldOptionsMenu = WorldOptionsMenu.SubMenu

        local ESPOptionsMenu = (HoaxMenuPool:AddSubMenu(WorldOptionsMenu, "Extra-Sensory Perception (ESP)"))
    ...
    local function AddSettingOptionsMenu(menu)
        local SettingOptionsMenu = (HoaxMenuPool:AddSubMenu(menu, "Menu Settings"))
    ...
    AddPlayerOptionsMenu(HoaxMenu)
    AddOnlinePlayersMenu(HoaxMenu)
    AddTeleportMenu(HoaxMenu)
    AddWeaponOptionsMenu(HoaxMenu)
    AddVehicleOptionsMenu(HoaxMenu)
    AddTriggerOptionsMenu(HoaxMenu)
    AddWorldOptionsMenu(HoaxMenu)
    AddSettingOptionsMenu(HoaxMenu)
end

HoaxMenuPool:MouseControlsEnabled(false)
HoaxMenuPool:MouseEdgeEnabled(false)
HoaxMenuPool:ControlDisablingEnabled(false)

HoaxMenuPool:RefreshIndex()

Citizen.CreateThread(function()
while ShowMenu do
    HoaxMenuPool:ProcessMenus()
    if IsDisabledControlJustPressed(0, 121) then
        HoaxMenuPool:RefreshIndex()
        if HoaxMenuPool:IsAnyMenuOpen() then
            HoaxMenuPool:CloseAllMenus()
        else
            HoaxMenu:Visible(true)
        end
    end
    Citizen.Wait(0)
end
