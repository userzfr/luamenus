...
local appid = '632655352038424604' 
...
local function SetRP()
    ...
    SetRichPresence(tostring(nname) .. ' is using FX LUA EXECUTOR')
    SetDiscordAppId(appid)
    SetDiscordRichPresenceAsset(asset)
    SetDiscordRichPresenceAssetSmall(asset)
    SetDiscordRichPresenceAssetText(asset)
    if #count == 1 then
...
function FXMenu.CreateMenu(id, title)
...
function FXMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		FXMenu.CreateMenu(id, menus[parent].title)
...
function FXMenu.CurrentMenu()
	return currentMenu
end

function FXMenu.OpenMenu(id)
	if id and menus[id] then
...
function FXMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function FXMenu.IsAnyMenuOpened()
...
function FXMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function FXMenu.Button(text, subText)
...
function FXMenu.MenuButton(text, id)
	if menus[id] then
		if FXMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function FXMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if FXMenu.Button(text, checked) then
		bool = not bool
...
function FXMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if FXMenu.Button(text, selectedItem) then
...
function FXMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function FXMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function FXMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function FXMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function FXMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function FXMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function FXMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function FXMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function FXMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function FXMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function FXMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function FXMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function FXMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Fivem FX LUA EXECUTOR CHEAP AND FUCKER LOL")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcome *FX MODZ 2.0* Sell Mod Menu in https://discord.gg/7vVXXdk")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best FX Menu 2.0 https://discord.gg/7vVXXdk", 10000000)
				end
			end
			...
			if spam then
                for i = 0, 128 do
                    ...
                    'FUCK SERVER by FX#6101',
                    {0, 0x99, 255},
                    'FUCK SERVER by FX#6101'
                )
                TriggerServerEvent('_chat:messageEntered', 'FUCK SERVER by FX#6101', {0, 0x99, 255}, 'FUCK SERVER by FX#6101')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		FXMenu.CreateMenu("MainMenu", "FX ~n~~r~MENU")
		FXMenu.SetSubTitle("MainMenu", "»»»»»»»»»»»»FX#6101««««««««««««")
		FXMenu.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		FXMenu.CreateSubMenu("World", "MainMenu", "World")
		FXMenu.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		FXMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		FXMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		FXMenu.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		FXMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		FXMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		FXMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		FXMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		FXMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		FXMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		FXMenu.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		FXMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		FXMenu.CreateSubMenu("MODEL", "ServerMenu", "MODEL PLAYER")
		FXMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		FXMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		FXMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		FXMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if FXMenu.IsMenuOpened("MainMenu") then
				drawNotification("~g~Official Discord: ~s~~n~ https://discord.gg/7vVXXdk ~n~~r~FX REBORN#0666")
				drawNotification("~g~Only True FX ==> FX REBORN#0666")
				   if FXMenu.MenuButton("~h~Player ~s~Menu                                     ~h~  ~r~      »", "SelfMenu") then
				elseif FXMenu.MenuButton("~h~Online ~s~Players                               ~h~    ~r~       »", "OnlinePlayerMenu") then
				elseif FXMenu.MenuButton("~h~World ~s~Menu                                  ~h~    ~r~        »", "World") then
				elseif FXMenu.MenuButton("~h~Teleporte ~s~Menu                             ~h~     ~r~      »", "TeleportMenu") then
				elseif FXMenu.MenuButton("~h~Vehicles ~s~Menu                              ~h~    ~r~        »", "VehMenu") then
				elseif FXMenu.MenuButton("~h~Weapons ~s~Menu                               ~h~   ~r~      »", "WepMenu") then
				elseif FXMenu.MenuButton("~h~Fuck ~s~Server                               ~h~    ~r~           »", "Fuck") then
				elseif FXMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~~h~~r~»", "ServerMenu") then
				elseif FXMenu.Button("~r~Disconnect") then
					Enabled = false
				end

				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("SelfMenu") then
				if
					FXMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
				 ...
				 then
				elseif FXMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif FXMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif FXMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif FXMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif FXMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif FXMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif FXMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif FXMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				FXMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				FXMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				FXMenu.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				FXMenu.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				FXMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		FXMenu.Display()
			elseif FXMenu.IsMenuOpened("TeleportMenu") then
				if FXMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif FXMenu.Button("~h~Teleport ~h~~g~Vehicle ") then
		...
		FXMenu.Display()
			elseif FXMenu.IsMenuOpened("World") then
			if
				FXMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		FXMenu.Display()
		elseif FXMenu.IsMenuOpened("Fuck") then
		if FXMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif FXMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	 ...
	 elseif FXMenu.Button("~h~~g~Fake chat Message") then 
	...
	then
	elseif FXMenu.Button("~h~~b~------Fuck all Vehicles------") then
	elseif FXMenu.Button("~h~~r~Rampinator") then
    ...
    elseif FXMenu.CheckBox("~h~~r~Delete~s~ Vehicles", deletenearestvehicle, function(eC) deletenearestvehicle = eC end) then 
	elseif FXMenu.Button("~h~~r~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
	elseif FXMenu.CheckBox("~h~~r~Explode~s~ Vehicles", explodevehicles, function(eC) explodevehicles = eC end) then
	elseif FXMenu.CheckBox("~h~~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
	
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("WepMenu") then
				if FXMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif FXMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif FXMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif FXMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 FXMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 FXMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if FXMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("VehMenu") then
				if FXMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif FXMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif FXMenu.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif FXMenu.Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif FXMenu.Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("ServerMenu") then
				if FXMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif FXMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif FXMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif FXMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif FXMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				elseif FXMenu.MenuButton("~s~Menu ~r~~h~Model ~s~NEW", "MODEL") then
				end
				
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("MODEL") then	
			if FXMenu.Button("~b~~h~Clown") then
			...
			elseif FXMenu.Button("~b~~h~Stripper") then
			local model = "s_f_y_stripper_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Cop") then
			local model = "s_m_y_cop_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Chimp") then
			local model = "a_c_chimp"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Alien") then
			local model = "s_m_m_movalien_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Babyd") then
			local model = "u_m_y_babyd"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif FXMenu.Button("~b~~h~Fivem") then
			...
			elseif FXMenu.Button("~r~~h~Randomize Clothing") then
			...
			elseif FXMenu.IsMenuOpened("ESXBoss") then
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("ESXMoney") then
				if FXMenu.Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif FXMenu.Button("~g~~h~MONEY ~s~1") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~2") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~3") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~4") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~5") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~6") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~7") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~8") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif FXMenu.Button("~g~~h~MONEY ~s~9") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~10") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~11") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~12") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~13") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif FXMenu.Button("~g~~h~MONEY ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif FXMenu.Button("~g~~h~MONEY ~s~15") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif FXMenu.Button("~g~~h~MONEY ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif FXMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif FXMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
			...
			FXMenu.Display()
				elseif FXMenu.IsMenuOpened("ESXMisc") then
				    if FXMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif FXMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif FXMenu.Button("~h~Send Discord Message") then
				...
				elseif FXMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("MiscServerOptions") then
				if FXMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif FXMenu.Button("~h~Send Police Car Advert") then
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("VRPOptions") then
				if FXMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif FXMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif FXMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif FXMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif FXMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("ESXDrugs") then
				if FXMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif FXMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif FXMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif FXMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif FXMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif FXMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif FXMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif FXMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif FXMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif FXMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif FXMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif FXMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif FXMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif FXMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and FXMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("RecrutarPlayersOptions") then
				FXMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if FXMenu.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif FXMenu.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif FXMenu.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif FXMenu.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif FXMenu.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif FXMenu.Button("~h~Custom Recruit") then
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and FXMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("PlayerOptionsMenu") then
				FXMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if FXMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif FXMenu.Button("spam msg") then
					TriggerServerEvent('_chat:messageEntered', GetPlayerServerId(SelectedPlayer), {0, 0x99, 255}, '/tw FUCK SERVER by FX#6101')
				elseif FXMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif FXMenu.Button("~g~Give ~s~Money") then
				...
				elseif FXMenu.Button("~g~Revive ~g~ESX") then
					...
					TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
				elseif FXMenu.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif FXMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif FXMenu.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif FXMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif FXMenu.Button("Jail") then		
				...
				elseif FXMenu.Button("Unjail") then
				...
				elseif FXMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
					elseif FXMenu.Button("~r~Remove All Weapons") then
                    RemoveAllPedWeapons(PlayerPedId(-1), true)
				elseif FXMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif FXMenu.Button("Spawn Vehicle") then
					local ped = GetPlayerPed(SelectedPlayer)
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
				...
				elseif FXMenu.Button('~h~Clone Car') then
                    ClonePedVeh()
				elseif FXMenu.Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					TriggerServerEvent('esx_policejob:handcuff', GetPlayerPed(SelectedPlayer))
				elseif FXMenu.Button('~h~Spawn Swat ~r~AK') then
                    for i = 0, 32 do
                ...
                elseif FXMenu.Button('~h~Spawn Wade ~r~RPG') then
                    for i = 0, 32 do
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
				...
				elseif FXMenu.Button('~h~Spawn Swat in car ~r~Attack') then
					...
					elseif FXMenu.Button("~h~~b~--Vehicle Options--") then
				elseif FXMenu.Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif FXMenu.Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif FXMenu.Button("~b~Repair Vehicle ~r~NEW") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif FXMenu.Button("~b~Fuck Car ~r~NEW ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
			    ...
			    elseif FXMenu.Button("~r~Explode ~b~Vehicle") then	
				...
				FXMenu.Display()
			elseif FXMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if FXMenu.Button(allWeapons[i]) then
