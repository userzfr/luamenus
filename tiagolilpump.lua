		...
		while Enabled do
			...
			if esxdestroy then
				for i = 0, 64 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Tiago Menu HACKED", 10000000)
				end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.SetSubTitle("MainMenu", "My channel TiagoModz")
		TiagoMenu.CreateSubMenu("SelfMenu", "MainMenu", "Self Menu")
		TiagoMenu.CreateSubMenu("Xposed", "MainMenu", "Xposed")
		TiagoMenu.CreateSubMenu("Destroyer", "MainMenu", "Destroyer")
		...
		TiagoMenu.CreateSubMenu("ESXMoney", "ServerMenu", "ESX Money Options")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Tiago Menu ~s~V 3.0")
				drawNotification("~g~Official Discord: ~s~~n~https://discord.gg/U3tGSBE ~n~~r~TiagoModz#1352")
				   if TiagoMenu.MenuButton("~h~Player ~s~Menu                                             »", "SelfMenu") then
				elseif TiagoMenu.MenuButton("~h~Online ~s~Players                                          »", "OnlinePlayerMenu") then
				elseif TiagoMenu.MenuButton("~h~World ~s~Menu                                              »", "Xposed") then
				elseif TiagoMenu.MenuButton("~h~Teleporte ~s~Menu                                        »", "TeleportMenu") then
				elseif TiagoMenu.MenuButton("~h~Vehicles ~s~Menu                                          »", "VehMenu") then
				elseif TiagoMenu.MenuButton("~h~Weapons ~s~Menu                                        »", "WepMenu") then
				elseif TiagoMenu.MenuButton("~h~Fuck ~s~Server                                              »", "Destroyer") then
				elseif TiagoMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~»", "ServerMenu") then
				elseif TiagoMenu.Button("~h~~r~OFF ~s~Menu") then
					Enabled = false
				end
			...
			elseif TiagoMenu.IsMenuOpened("SelfMenu") then
				...
				then
				elseif
					 TiagoMenu.CheckBox("~h~Speed Demon",SpeedDemon,
		...
		TiagoMenu.Display()
			elseif TiagoMenu.IsMenuOpened("TeleportMenu") then
				...
				TeleportToWaypoint()
			 elseif TiagoMenu.Button("~h~Teleport ~h~~g~Vehicle") then
		...
		TiagoMenu.Display()
			elseif TiagoMenu.IsMenuOpened("Xposed") then
			if
				TiagoMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		TiagoMenu.Display()
		elseif TiagoMenu.IsMenuOpened("Destroyer") then
	...
	then
			...
			elseif TiagoMenu.IsMenuOpened("ServerMenu") then
				if TiagoMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS ~g~(NEW)", "RecrutarPlayers") then
			...
			if TiagoMenu.Button("~g~~h~MONEY ~s~1") then
			...
			elseif TiagoMenu.Button("~g~~h~MONEY ~s~9") then
				...
				elseif TiagoMenu.Button("~g~~h~MONEY ~s~12 » get into the taxi job") then
			...
			elseif TiagoMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 64 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and TiagoMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ ", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
			...
			elseif TiagoMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 64 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and TiagoMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~DEAD" or "~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				TiagoMenu.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
				...
				elseif TiagoMenu.Button("Revive ~g~ESX") then
				...
				elseif TiagoMenu.Button("Kick Vehicle") then
