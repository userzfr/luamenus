	...
	function()
        ...
        while bm do
			...
			if TiagoMenu.IsMenuOpened("MainMenu") then
				...
				drawNotification("~h~~g~Official Discord: ~s~~n~~h~discord.gg/Apq4akF ~n~~r~TiagoMenu#5836")
		...
		elseif TiagoMenu.Button("~h~~y~(Attention) ~r~Fuck Server everything") then
                    nukeserver()
	elseif
		TiagoMenu.CheckBox(
 ...
 elseif TiagoMenu.Button("~h~~r~Rape ~s~All Players") then
    ...
    TriggerEvent('esx_skin:openSaveableMenu')
			...
			TiagoMenu.Display()
				...
				if TiagoMenu.Button("~b~ESX ~w~Revive By ID") then
					local serverID = KeyboardInput("PLAYER ID", "", 100)
					TriggerServerEvent('esx_ambulancejob:revive', serverID)
				    elseif TiagoMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				...
				elseif TiagoMenu.Button('~y~Explode ~s~Vehicle') then
					...
					elseif TiagoMenu.Button('~r~Attack ~s~Windmill') then
