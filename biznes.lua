...
function SkazaMenu.CreateMenu(id, title)
	...
	Citizen.CreateThread(
		function()
			while true do
				...
				menus[id].titleBackgroundColor = {r = 65, g = 0, b = 87, a = 225}
	...
	--menus[id].menuFocusBackgroundColor = { r = 2, g = 36, b = 6, a = 255 }
	menus[id].menuBackgroundColor = {r = 195, g = 0, b = 255, a = 100}
...
function SkazaMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		SkazaMenu.CreateMenu(id, menus[parent].title)
...
function SkazaMenu.CurrentMenu()
	return currentMenu
end

function SkazaMenu.OpenMenu(id)
	if id and menus[id] then
...
function SkazaMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function SkazaMenu.IsAnyMenuOpened()
...
function SkazaMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function SkazaMenu.Button(text, subText)
...
function SkazaMenu.MenuButton(text, id)
	if menus[id] then
		if SkazaMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function SkazaMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if SkazaMenu.Button(text, checked) then
		bool = not bool
...
function SkazaMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if SkazaMenu.Button(text, selectedItem) then
...
function SkazaMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function SkazaMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function SkazaMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function SkazaMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function SkazaMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function SkazaMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function SkazaMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function SkazaMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function SkazaMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function SkazaMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function SkazaMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function SkazaMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function SkazaMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
	function()
		...
		SkazaMenu.CreateMenu("MainMenu", "<FONT COLOR='#ff00ea'>BRUTAN V9.5")
		SkazaMenu.SetSubTitle("MainMenu", "∑ NEW BRUTAN MENU HYPE ")
		SkazaMenu.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		SkazaMenu.CreateSubMenu("World", "MainMenu", "World")
		SkazaMenu.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		SkazaMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		SkazaMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		SkazaMenu.CreateSubMenu("TworcyMenu", "MainMenu", "tworcy")
		SkazaMenu.CreateSubMenu("TeleportMenu", "MainMenu", "TeleportRawQ Menu")
		SkazaMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		SkazaMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		SkazaMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		SkazaMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		SkazaMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		SkazaMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		SkazaMenu.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		SkazaMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		SkazaMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		SkazaMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		SkazaMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		SkazaMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if SkazaMenu.IsMenuOpened("MainMenu") then
			    drawNotification("<FONT COLOR='#ff00f2'>BRUTAN MENU")
				drawNotification("<FONT COLOR='#ff00f2'>∑ JOIN ME ~s~~n~ON DISCORD ~n~~r~https://discordapp.com/invite/tCEajtn")
				   if SkazaMenu.MenuButton("~h~» Player ~s~Menu                                     ~h~  ~r~      ", "SelfMenu") then
				elseif SkazaMenu.MenuButton("~h~» Online ~s~Players                               ~h~    ~r~       ", "OnlinePlayerMenu") then
				elseif SkazaMenu.MenuButton("~h~» World ~s~Menu                                  ~h~    ~r~        ", "World") then
				elseif SkazaMenu.MenuButton("~h~» Teleporte ~s~Menu                             ~h~     ~r~      ", "TeleportMenu") then
				elseif SkazaMenu.MenuButton("~h~» Vehicles ~s~Menu                              ~h~    ~r~        ", "VehMenu") then
				elseif SkazaMenu.MenuButton("~h~» Weapons ~s~Menu                               ~h~   ~r~      ", "WepMenu") then
				elseif SkazaMenu.MenuButton("~h~» Fuck ~s~Server                               ~h~    ~r~           ", "Fuck") then
				elseif SkazaMenu.MenuButton("~h~<FONT COLOR='#00ff33'>∑ MONEY  ∑                                         ~s~~h~~r~", "ServerMenu") then
				elseif SkazaMenu.Button("<FONT COLOR='#ffffff'>Disconnect") then
					Enabled = false
				end
				
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("bizness") then

				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("SelfMenu") then
				if
					SkazaMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
				 ...
				 then
				elseif SkazaMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif SkazaMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif SkazaMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif SkazaMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif SkazaMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif SkazaMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif SkazaMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif SkazaMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				SkazaMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				SkazaMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				SkazaMenu.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				SkazaMenu.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				SkazaMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("TeleportMenu") then
				if SkazaMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif SkazaMenu.Button("~h~Teleport ~h~~g~Vehicle ~r~FIX ") then
		...
		SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("World") then
			if
				SkazaMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		SkazaMenu.Display()
		elseif SkazaMenu.IsMenuOpened("Fuck") then
		if SkazaMenu.Button('~h~~p~#~s~ Spawn Peds', 'SpawnPeds') then
		end
		if SkazaMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif SkazaMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("WepMenu") then
				if SkazaMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif SkazaMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif SkazaMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif SkazaMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 SkazaMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 SkazaMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if SkazaMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("VehMenu") then
				if SkazaMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif SkazaMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif SkazaMenu.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif SkazaMenu.Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif SkazaMenu.Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("ServerMenu") then
				if SkazaMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif SkazaMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif SkazaMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif SkazaMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif SkazaMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("ESXBoss") then
				if SkazaMenu.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif SkazaMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("ESXMoney") then
				if SkazaMenu.Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif SkazaMenu.Button("~g~~h~MONEY, CAUTION ~s~1") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, TRUCKER ~s~2") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, ADMIN BANK ~s~3") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, ADMIN ~s~4") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, GOPOSTAL ~s~5") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, BANKSEC ~s~6") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, SLOT ~s~7") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, LSC ~s~8") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif SkazaMenu.Button("~g~~h~MONEY, SLOT ~s~9") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, 999 ~s~10") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, TRUCKERJOB ~s~11") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, here ~s~12") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, TAXI) ~s~13") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif SkazaMenu.Button("~g~~h~MONEY, TAXI ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif SkazaMenu.Button("~g~~h~MONEY, GARBAGEJOB ~s~15") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif SkazaMenu.Button("~g~~h~MONEY, Salary ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif SkazaMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif SkazaMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
			...
			SkazaMenu.Display()
				elseif SkazaMenu.IsMenuOpened("ESXMisc") then
				    if SkazaMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif SkazaMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif SkazaMenu.Button("~h~Send Discord Message") then
				...
				elseif SkazaMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("MiscServerOptions") then
				if SkazaMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif SkazaMenu.Button("~h~Send Police Car Advert") then
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("VRPOptions") then
				if SkazaMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif SkazaMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif SkazaMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif SkazaMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif SkazaMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("ESXDrugs") then
				if SkazaMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif SkazaMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif SkazaMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif SkazaMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif SkazaMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif SkazaMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif SkazaMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif SkazaMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif SkazaMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif SkazaMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif SkazaMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif SkazaMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif SkazaMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif SkazaMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and SkazaMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("RecrutarPlayersOptions") then
				SkazaMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if SkazaMenu.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif SkazaMenu.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif SkazaMenu.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif SkazaMenu.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif SkazaMenu.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif SkazaMenu.Button("~h~Custom Recruit") then
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and SkazaMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("PlayerOptionsMenu") then
				SkazaMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if SkazaMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif SkazaMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif SkazaMenu.Button("~g~Give ~s~Money") then
				...
				elseif SkazaMenu.Button("~r~Remove ~s~Money") then
				...
				elseif SkazaMenu.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif SkazaMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif SkazaMenu.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif SkazaMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif SkazaMenu.Button("Jail") then		
				...
				elseif SkazaMenu.Button("Unjail") then
				...
				elseif SkazaMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif SkazaMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif SkazaMenu.Button("Spawn Vehicle") then
				...
				elseif SkazaMenu.Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					elseif SkazaMenu.Button("~h~~b~--Vehicle Options--") then
				elseif SkazaMenu.Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif SkazaMenu.Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif SkazaMenu.Button("~b~Napraw Pojazd ~r~NOWE") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif SkazaMenu.Button("~b~Rozjeb Samochod ~r~NOWE ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				SkazaMenu.Display()
			elseif SkazaMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if SkazaMenu.Button(allWeapons[i]) then
